		...
		<div className="relative min-h-screen flex items-center justify-center overflow-hidden">
			...
			<div className="relative z-10 max-w-5xl mx-auto px-4">
				<div className="text-center space-y-6">
					...
					<div className="space-y-4 sm:space-y-6 mt-6 sm:mt-8">
						<div className="flex flex-col items-center gap-4">
							<div className="flex items-center gap-3">
								...
								<h2 className="text-xl sm:text-2xl font-light tracking-wide text-blue-400">Développeur Python</h2>
								<span className="h-[1px] w-8 sm:w-12 bg-blue-500"></span>
							</div>
							<p className="text-base sm:text-lg text-gray-400 max-w-2xl px-4 sm:px-0">
								Création d&apos;expériences numériques exceptionnelles avec des technologies web modernes et des solutions backend robustes.
							</p>
						</div>

						{/* Tech Stack Pills */}
						<div className="flex flex-wrap justify-center gap-2 sm:gap-3 mt-6 sm:mt-8 px-4 sm:px-0">
							{['Python', 'Django', 'FastAPI', 'Javascript', 'Vue.js','Java','Spring', 'Docker','Azure','Gitlab CI','...'].map((tech) => (
								<span
									key={tech}
					...
					{[
						{ name: 'GitHub',    Icon: FaGithub,   href: 'https://github.com/Jilvo' },
						{ name: 'LinkedIn',  Icon: FaLinkedin, href: 'https://www.linkedin.com/in/julien-millot/' },
						{ name: 'Mon CV',   Icon: FaFilePdf ,  href: '/cv.pdf' },
					].map(({ name, Icon, href }) => (
						<a
						key={name}
						href={href}
						...
						aria-label={name}
						>
						<Icon className="w-6 h-6 sm:w-8 sm:h-8 inline-block group-hover:scale-110 transition-transform" />
						</a>
					))}
					</div>
				</div>
			...
			<div className="absolute bottom-8 inset-x-0 flex flex-col items-center animate-bounce">
				<span className="text-gray-400 text-sm mb-2 text-center">Scrollez pour voir plus</span>
